 ...
 */
public class ViterbiTriPOSTaggerTester {
	...
	static class POSTagger {
		...
		private Trellis<State> buildTrellis(List<String> sentence) {
			...
			trellis.setTransitionCount(State.getStartState(), State.getStartState(), 0);
			for (int position = 0; position <= sentence.size() + 1; position++) {
				...
				for (State state : states) {
					...
					for (String tag : tagScores.keySet()) {
						State s = trellis.getBackwardTransitions(state).argMax();
						double delta = trellis.getBackwardTransitions(state).getCount(s);
						...
						if (!trellis.backwardTransitions.containsKey(nextState))
							...
							State s0 = trellis.getBackwardTransitions(nextState).argMax();
							double delta0 = trellis.getBackwardTransitions(state).getCount(s0);
							if (delta0 > res) 
								trellis.setTransitionCount(state, nextState, res);
						}
		...
		public Counter<String> getLogScoreCounter(
			...
			for (String tag : tagCounter.keySet()) {
				...
				if (wordSet.containsKey(word))
					...
					p_wt = unknownWordTags.getCount(tag) / unknownWordTags.totalCount();
				...
				if (triTag.containsKey(tri)) {
					p_gram = triTag.getCount(tri) / (biTag.getCount(bi)
							+ uniTag.size());
				} else
					p_gram = 1 / (biTag.getCount(bi)
							+ uniTag.size());
