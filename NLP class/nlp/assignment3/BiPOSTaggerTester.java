 ...
 */
public class BiPOSTaggerTester {
	...
	static class MostFrequentTagScorer implements LocalTrigramScorer {
      ...
      String previoudWord = localTrigramContext.getPreviousWord();
      String previoudTag = localTrigramContext.getPreviousTag();
      ...
      for (String tag : tagCounter.keySet()) {
    	  ...
    	  else p_wt = 0.000000000000001;
    	  List<String> bi = new ArrayList<String>();
    	  bi.add(previoudTag);
    	  bi.add(tag);
    	  double p_gram = 0;
    	  if (uniTag.containsKey(previoudTag)) {
    		  if (biTag.containsKey(bi)) {
    			  p_gram = biTag.getCount(bi) / (uniTag.getCount(previoudTag) + uniTag.size());
    		  }
    		  else 
    			  p_wt = unknownWordTags.getCount(tag) / unknownWordTags.totalCount();
    	  }
    	  else p_gram = 1 / uniTag.size();
      ...
      return logScoreCounter;
		...
		public void train(
			...
			for (LabeledLocalTrigramContext labeledLocalTrigramContext : labeledLocalTrigramContexts) {
				...
				if (!labeledLocalTrigramContext.previousPreviousTag.equals(STOP_TAG))
