...
import nlp.assignment3.MaximumEntropyClassifier;
import nlp.classify.*;
...
//
			...
			if (characters.length < 10)
				features.incrementCount("length<10", 1);
			if (characters.length > 20)
				features.incrementCount("length>20", 1);
			// # of space
			int spaceNum = 0;
			...
			features.incrementCount("spaceNum", spaceNum);
			// # of -
			int hyphenNum = 0;
			for (int i = 1; i < characters.length; i++) {
				if (characters[i] == '-')
			...
			features.incrementCount("hyphenNum", hyphenNum);
			// if last character digit
			char lastCharacter = characters[characters.length - 3];
			if (lastCharacter >= '0' && lastCharacter <= '9')
				features.incrementCount("lastDigit", 1.0);
			// if last character dot
			if (lastCharacter == '.')
				features.incrementCount("lastDot", 1.0);
			// if is one word
			if (spaceNum == 0)
				features.incrementCount("isOne", 1.0);
			// start with the
			if (name.substring(0, 3).equals("The"))
				features.incrementCount("start_with_The", 1.0);
			// start with a 
			if (name.substring(0,2).equals("A "))
				features.incrementCount("start_with_A", 1.0);
			// start with digit
			char firstCharacter = name.charAt(0);
			if (firstCharacter >= '0' && firstCharacter <= '9')
				features.incrementCount("firstDigit", 1.0);			
			return features;
		}
	}
	...
	public static void generateConfusion(String label, String trueLabel) {
		int realLabelIndex = 0;
		int predictLabelIndex = 0;
		switch (trueLabel) {
		case "place":
			realLabelIndex = 0;
			break;
		...
		confusion[realLabelIndex][predictLabelIndex]++;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		} else if (model.equalsIgnoreCase("cunigram")) {
			classifier = new CUniGramClassifier.Factory<String, String>()
					.trainClassifier(trainingData);
		} else if (model.equalsIgnoreCase("cbigram")) {
			classifier = new CBiGramClassifier.Factory<String, String>()
					.trainClassifier(trainingData);
