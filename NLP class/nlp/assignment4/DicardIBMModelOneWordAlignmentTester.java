 ...
 */
public class DicardIBMModelOneWordAlignmentTester {
		...
		public void train(List<SentencePair> trainingSentencePair) {
			...
			while (delta > threshold) {
				...
				for (SentencePair sp : trainingSentencePair) {
					...
					int eSize = el.size();
					int fSize = fl.size();
					Counter<Integer> pMap = new Counter<Integer>();
					int countSum = (int) Math.pow(eSize, fSize);
					// calculate all the possibility of the sentence
					for (int times = 0; times < countSum; times++) {
						double oneP = 1;
						pMap = new Counter<Integer>();
						for (int fp = 0; fp < fSize; fp++) {
							int ep = generatePos(eSize, fp, times);
//							if (ep ==5) {
//								System.out.print(1);
//							}
							double thisP = lastP.getCount(el.get(ep),
									fl.get(fp));
							oneP *= thisP;
						}
						pMap.incrementCount(times, oneP);
					}
					pMap.normalize();
					...
					for (int times = 0; times < countSum; times++) {
						for (int fp = 0; fp < fSize; fp++) {
							int ep = generatePos(eSize, fp, times);
							currentP.incrementCount(el.get(ep), fl.get(fp),
									pMap.getCount(times));
						}
					}
				}
		...
		private int generatePos(int eSize, int fp, int times) {
			int pos = 0;
			
				pos = (int) (times / (Math.pow(eSize, fp)) % eSize);
						
			
				//pos = (int) (times / (Math.pow(eSize, fp))+1);
			return pos;
		}
	}
