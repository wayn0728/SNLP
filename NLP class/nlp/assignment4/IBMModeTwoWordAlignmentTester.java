 ...
 */
public class IBMModeTwoWordAlignmentTester {
	...
	static class ModelTwoAligner implements WordAligner {
		...
		public Alignment alignSentencePair(SentencePair sentencePair) {
			...
			for (int fp = 0; fp < fSize; fp++) {
				...
				for (int ep = -1; ep < eSize; ep++) {
					if (ep == -1) {
						double thisP = (p.getCount("#", fw) * indice
								.getCount(ep, construct(fp, eSize, fSize)));
						sumP += thisP;
					} else {
						String ew = el.get(ep);
						double count = p.getCount(ew, fw);
						double indiceCount = indice
								.getCount(ep, construct(fp, eSize, fSize));
				...
				for (int ep = -1; ep < eSize; ep++) {
					double thisP = 0;
					if (ep == -1) {
						thisP = (p.getCount("#", fw) * indice
								.getCount(ep, construct(fp, eSize, fSize)));
						thisP /= sumP;
						
					} else {
						String ew = el.get(ep);
						thisP = (p.getCount(ew, fw) * indice
								.getCount(ep, construct(fp, eSize, fSize)));
						thisP /= sumP;
					}
					if (thisP > maxP) {
		...
		public void trainModel2(List<SentencePair> trainingSentencePair,List<SentencePair> testSentencePair) {
			// count length range
					...
					for (int fp = 0; fp < fSize; fp++) {
						...
						for (int ep = -1; ep < eSize; ep++) {
							if (ep == -1) {
								sumP += (lastP.getCount("#", fw) * lastIndice
										.getCount(-1, construct(fp, eSize, fSize)));
							} else {
								String ew = el.get(ep);
						...
						for (int ep = -1; ep < eSize; ep++) {
							if (ep == -1) {
								...
								tempP /= sumP;
								currentP.incrementCount("#", fw, tempP);
								currentIndice.incrementCount(ep, construct(fp,eSize,fSize), tempP);
							} else {
								String ew = el.get(ep);
								double tempP = (lastP.getCount(ew, fw) * lastIndice
										.getCount(ep, construct(fp, eSize, fSize)));
								tempP /= sumP;
								if (ep == 0 && fp == 0 && eSize == 3 && fSize ==3) {
									int a = 1;
								}
								currentP.incrementCount(ew, fw, tempP);
								currentIndice.incrementCount(ep, construct(fp,eSize,fSize), tempP);
